...
| `auth-and-settings_accepted.html` | Signup/invite (1.4), password reset (1.5), SSO not-provisioned (1.3), 404 (1.7), user settings (2.2) | Accepted. Note: shown signup card is the *invite-accept* variant (email optional); the *open self-signup* form requires a **confirmed email** as bot defense. Username is read-only in settings (school-assigned). |
