...
- **Add subject** creates a new subject with a name and a slug (generated
  from the name if left blank, with a numeric suffix on collision).
- Existing subjects can be renamed or removed from their row.
- The list shows each subject's **course count** so you can see how widely
  it is used before deleting it; deleting a subject only unlinks it from
  its courses — the courses and their content are untouched.
