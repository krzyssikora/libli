...
e.g. `#147E78`), the **default theme** (light/dark), and which
**languages** are enabled for the platform's own UI, with one of them
chosen as the **default language**. At least one language must stay
enabled, and the default must be one of the enabled ones.

## Access

Controls who can sign up and where from:

- **Signup policy** — whether self-service sign-up is open, restricted, or
  disabled; invitations (see [Invitations](invitations)) work regardless of
  this setting.
