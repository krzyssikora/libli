...
- **No other question types.** Only `ChoiceQuestionElement` (single + multi). Other 8 types → 2b/2d.
...
- `Choice` is a plain model (not an element / not in the GFK). Choice text is **plain text + math** (rich text per choice is out of scope).
...
MarkResult
  ...
  reveal:   <payload>       # presentation data for the feedback render
                            # for ChoiceQuestionElement: the set of correct choice ids
```
...
- **`Choice` inline formset** — `inlineformset_factory(ChoiceQuestionElement, Choice, fields=["text", "is_correct", "order"], extra=..., min_num=2, validate_min=True)`. The editor partial renders the question form together with the choice formset (add/remove rows).
- **Validation (form/formset `clean()`):**
  - ≥ 2 choices.
  - ≥ 1 correct choice always.
  - For `multiple=False`: **exactly 1** correct choice.
  - Errors surface as form errors; the editor's existing 422 re-render path shows the bound form + formset.
...
- The **add-menu** offers **two cards** — "Single choice" and "Multiple choice" — that both create a `ChoiceQuestionElement`, differing only in the `multiple` value they seed at creation. So `element_add` must accept an add-key that carries the `multiple` seed (e.g. `"choice-single"` / `"choice-multi"`), while `element_save` / edit resolves back through the single `"choicequestion"` form key.
- This is the one place the current 1:1 `type_key ↔ model` assumption bends; the plan must pin the add-key→seed and edit-key→form mapping precisely. The choice formset is wired in alongside the form in `element_add` (render-only) and `element_save` (create-on-first-save via the `new` sentinel) — saved atomically with the question.

### 3.3 Marking rules (2a)

`ChoiceQuestionElement.mark(answer)` (answer = the set of selected choice ids):
...
- stem (sanitized HTML, KaTeX-rendered) + choices as `<input type="radio">` (single) or `<input type="checkbox">` (multi); labels = choice text.
...
2. Read submitted choice ids from POST; **validate them against this question's own `choices` queryset** (drop foreign/forged ids — treat as not-selected, never error-leak).
3. Call `question.mark(answer)` → `MarkResult`. **Nothing is persisted.**
4. Render the feedback fragment `_question_feedback.html`: verdict (✓/✗), revealed correct choice(s), explanation (if any), and the form re-shown for free retry.
...
- **No-JS path:** the same `<form>` does a full POST to `check_answer`; the view re-renders the **lesson page** with that question's feedback block shown inline and its inputs repopulated (selected ids echoed from POST). All state lives in the request — consistent with the ephemeral model.

### 3.6 Lesson view wiring & progress

- Add a `has_questions` flag (parallel to `has_math`/`has_html`) to gate the `question.js` include.
- KaTeX: questions whose stem/choices contain math are covered by extending the existing `has_math` scan to question content (so the existing KaTeX gate fires).
...
- **Unit:**
  - `mark()` for single & multi (all-or-nothing; forged-id drop; fraction values).
...
- `courses/views_manage.py` — add-key→`multiple` seed in `element_add`/`element_save`; formset wiring.
