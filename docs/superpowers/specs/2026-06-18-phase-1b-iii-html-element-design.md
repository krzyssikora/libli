...
**Relative-URL resolution (opaque origin):** a `srcdoc` document's relative URLs have no intrinsic base, so the document injects `<base href="<APP_ORIGIN>/">` (same trusted origin as the CSP, §2.4). This makes author root/path-relative resource URLs (e.g. `<img src="/media/…">`) resolve to the libli origin, which the CSP `img-src <APP_ORIGIN>` then permits. **It does NOT fix KaTeX fonts:** the vendored `katex.min.css` declares fonts as `url(fonts/KaTeX_*.woff2)` *relative to the CSS file* (`/static/courses/vendor/katex/`); once that CSS is inlined as a `<style>` block, those relative URLs resolve against `<base>` → `<APP_ORIGIN>/fonts/…` (a 404, since the real path is `<APP_ORIGIN>/static/courses/vendor/katex/fonts/…`). KaTeX font URLs are therefore **rewritten to absolute static paths** when the CSS blob is prepared — see §4.3 (this is the specified default, not a fallback).
...
- When preparing the CSS constant, **rewrite each `url(fonts/KaTeX_*.woff2)` to the absolute static URL** `"<APP_ORIGIN>/" + static("courses/vendor/katex/fonts/…")` (i.e. `…/static/courses/vendor/katex/fonts/…`). This is the **specified default**, because the inlined CSS otherwise resolves font URLs against `<base>` to a 404 (§4.1).
