...
- **Course-wide CSS/JS:** two code textareas on the course settings/builder surface (alongside existing course fields). PA/owner-gated by the established manage predicate (`owner OR has_perm("courses.change_course")`). **Concrete edit:** add `html_css`/`html_js` to `CourseForm.Meta.fields` (`courses/forms.py`) with monospace `Textarea` widgets, and render them in `templates/courses/manage/course_form.html` (the existing `CourseForm.Meta.fields` is an explicit fixed list, so this is required, not automatic).
...
**Concrete machinery edits required (verified against the code — the existing machinery hardcodes the five types in three places that will reject `"html"` otherwise):**

1. Add `"html"` to the type-key allowlist tuple in **both** `views_manage.element_add` and `views_manage.element_save` (each currently `("text", "image", "video", "iframe", "math")`).
2. Add `HtmlElementForm` to `element_forms.FORM_FOR_TYPE` under key `"html"`.
3. `HtmlElementForm` is a **plain `ModelForm(fields=["html"])`** requiring **no** `course=`/`unit=` at construction — so the `extra = {"course": …} if type_key in ("image", "video")` branches in `_render_open_form`/`element_form`/`builder.save_element` stay **untouched**. Course/unit are needed only at **render/assembly** time (§4.2), never at form-construction time. The form must add **no** tag-stripping `clean_html` (§3.1).
4. The type→partial dispatch resolves `_edit_html.html` by the existing `_edit_<type>.html` convention. The edit-existing flow derives the type key via `element_form`'s `__name__.lower().replace("element", "")` (`views_manage.py:746`), which yields `"html"` for `HtmlElement` (`"htmlelement"` → `"html"`) — correct, so both add and edit route properly.
...
**Relative-URL resolution (opaque origin):** a `srcdoc` document's relative URLs have no intrinsic base, so the document injects `<base href="<APP_ORIGIN>/">` (same trusted origin as the CSP, §2.4). This makes author root/path-relative resource URLs (e.g. `<img src="/media/…">`) resolve to the libli origin, which the CSP `img-src <APP_ORIGIN>` then permits. **It does NOT fix KaTeX fonts:** the vendored `katex.min.css` declares each face with three `src:` entries — `url(fonts/KaTeX_*.woff2)`, `url(fonts/KaTeX_*.woff)`, and `url(fonts/KaTeX_*.ttf)` (20 of each) — all *relative to the CSS file* (`/static/courses/vendor/katex/`); once that CSS is inlined as a `<style>` block, those relative URLs resolve against `<base>` → `<APP_ORIGIN>/fonts/…` (a 404, since the real path is `<APP_ORIGIN>/static/courses/vendor/katex/fonts/…`). KaTeX font URLs are therefore **rewritten to absolute static paths** when the CSS blob is prepared — see §4.3 (this is the specified default, not a fallback).
...
**Course-JS-vs-math ordering (authoring note).** Two facts, kept consistent with the §4.3 gating predicate:
- KaTeX is injected **iff the element's own `html` contains `\(` or `\[`** (§4.3 scans only the static `html`, not the JS). So math that course JS *injects* into the DOM is typeset by the post-course-JS auto-render pass **only when that element's `html` also contains a delimiter** (so KaTeX is present). If an element's static `html` has no delimiter, KaTeX is not injected and JS-injected math will **not** typeset — authors who generate math via JS must include at least one delimiter in the static `html` (a hidden `\()` token suffices) to trigger injection.
- course JS must **not** read already-typeset `.katex` output at load time (it runs before the auto-render pass, so that output doesn't exist yet). Code needing typeset math should run after typesetting.
...
- **Query-count guarantee is a separate, required change in every feeding view** (not in the tag). `render_element`'s `element.unit.course` is query-free *only if* the calling view's element queryset pre-loads that chain. Verified state: the **lesson view** (`views.py:52`) currently does `node.elements...prefetch_related("content_object")` — it must **also** add `select_related("unit__course")` (the two compose); the **editor** path must do likewise at its single shared queryset site **`_editor_rows(unit)`** (`views_manage.py:578`, currently `unit.elements.select_related("content_type")…`) — this one helper feeds **both** the editor page and the `_render_editor_fragments` preview swap, so the one edit covers both. Without these the output is still correct but incurs N+1 FK lookups, silently violating this goal.
...
KaTeX runs with `throwOnError:false` (raw LaTeX left on failure).

**Inlining, font URLs, and caching (resolves the C1 path bug + per-render cost).** KaTeX CSS + JS (`katex.min.css`, `katex.min.js`, `contrib/auto-render.min.js`) are inlined into the srcdoc. To make this correct and cheap:
- **Read once at module import** into module-level constants (≈275 KB JS + ≈23 KB CSS) — never re-read from disk/staticfiles per element render. The per-iframe `srcdoc` duplication of these blobs is the already-accepted cost (§3.2/§9); the *read* is one-time.
- When preparing the CSS constant, **rewrite every `url(fonts/…)` occurrence — woff2, woff, and ttf alike** (match `url(fonts/` regardless of extension, not just `.woff2`) — to the absolute static URL `"<APP_ORIGIN>/" + static("courses/vendor/katex/fonts/…")` (i.e. `…/static/courses/vendor/katex/fonts/…`). This is the **specified default**, because the inlined CSS otherwise resolves font URLs against `<base>` to a 404 (§4.1). (A woff2-only rewrite would break the woff/ttf fallbacks that older browsers select.)
- **CORS:** the opaque-origin iframe fetching those fonts from `<APP_ORIGIN>` is a cross-origin font request, which requires `Access-Control-Allow-Origin` on the font responses. **WhiteNoise (in use) sets `Access-Control-Allow-Origin: *` on static files by default**, satisfying this; verify it is not stripped in the deploy. Vendoring task: add `auto-render.min.js` alongside the existing `katex.min.js`/`katex.min.css`. **KaTeX fonts:** the vendored `katex.min.css` references woff2 font files by URL; these resolve to the libli app origin and are permitted by the CSP `font-src <APP_ORIGIN>` entry (§2.4) — *not* by `'self'`, which is inert under the opaque origin. (Alternative if origin injection proves awkward: ship a data-URI-inlined KaTeX font CSS so fonts need no `font-src` host. Default is the app-origin approach.)

### 4.4 Resize bridge (client)

- **In-sandbox reporter** (injected): measures content height from **`document.body.scrollHeight`** (or `body.getBoundingClientRect().height`) — **not** from `documentElement`'s box, which tracks the *iframe's* applied height and creates a feedback floor where the document can never report shorter than it was last sized to (a known auto-resize footgun). It posts the §2.5 contract message to `window.parent`: (a) **unconditionally on initial load** (so a freshly-swapped preview iframe always reports, even when content shrank and triggers no observer change), (b) on every `ResizeObserver` change (steady-state backstop), (c) after the KaTeX auto-render pass completes, and (d) on `document.fonts.ready` — because KaTeX typeset and web-font loading can finish *after* initial layout, a single end-of-script report would under-measure tall/math content. **Author-CSS caveat:** measuring `body.scrollHeight` avoids the `documentElement` feedback floor, but author CSS that *pins* body height (`body{height:100vh}` / `height:100%`) can still make `body.scrollHeight` track the iframe's applied height; the reporter should take `Math.max(body.scrollHeight, body.getBoundingClientRect().height)` and the §6.4 authoring guidance should note that pinning `body{height}` inside an HTML element breaks auto-sizing.
...
- A non-`.html-el` iframe (e.g. an `iframeelement`/GeoGebra embed) posting the `{type:"libli:htmlel:height"}` shape is **not** resized — asserts the §4.4 `.html-el`-only sender restriction.
...
- **Also extract:** the §2.6 student-facing visual-frame label (e.g. "interactive content") and any author help/placeholder text explaining the seed JS and CSS/JS fields.
...
- **Field/`srcdoc` size (accepted unbounded):** the assembled `srcdoc` duplicates course CSS/JS + seed + inline KaTeX per element, so a math-heavy page can carry several large attributes. No length cap is imposed on the four text fields in 1b-iii (recorded decision, same spirit as the per-iframe KaTeX-duplication acceptance); revisit only if real content hits browser/DB limits.
