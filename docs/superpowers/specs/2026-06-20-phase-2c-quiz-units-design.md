...
- Keyed on `Element` (the 1a GFK join-row model — `Element.unit` FK to the unit `ContentNode`, `content_type`/`object_id` to the concrete element), not the concrete question — a stable identity independent of question type, matching how `check_answer` already locates a question (`Element` → `content_object`). The `Element.unit` FK is also the scoping/IDOR guard used in §3.1 step 2.
...
3. `answer = question.build_answer(request.POST)` — the **existing** per-type `build_answer(self, post)` that 2a/2b's `check_answer` already calls (concrete on each question model). **Empty-answer guard:** if `answer` is empty/blank (per type — empty choice set, blank string, all-blank fill-blanks), **reject without burning an attempt** (no `Attempt` row, `attempt_count` unchanged) and return a validation message ("Enter an answer"), branching on `_wants_fragment` like every other path: the **JS-fragment** path returns just the offending question's feedback partial showing the validation message; the **no-JS** path re-renders the full `quiz_unit.html` with the complete rehydrated context (§4.1) plus the validation message anchored to the offending question (by its `element.pk`). Uniform across types; a blank submit never wastes a capped attempt.
...
- Render the results summary: total `score / max_score` plus per-question outcome. The summary **iterates the unit's question `Element`s in document order and left-joins each `QuestionResponse`** (it does *not* iterate response rows — that would drop unanswered questions). Outcome classification keys on the question's **current** `marking_mode` first (so a question switched `[A]→[N]` mid-quiz shows the `[N]` outcome and its stale stored `fraction`/`earned_marks` are simply ignored, mirroring the scoring discriminator): **`[N]`** → "answer recorded" (or "not answered" if no response); **`[R]`** → "awaiting review"; **`[A]`** → correct (`earned == possible`) / partial (`0 < earned < possible`, shown as **`earned / possible` marks** via §3.6 — *not* a blank count `n/m`, which isn't reconstructable from the stored `fraction`) / incorrect (`earned == 0` with a response) / **not answered** (no response). The summary then reveals all answers + explanations.
...
- **Stored fraction (single source of truth).** `f = Decimal(str(result.fraction)).quantize(Decimal("0.0001"), ROUND_HALF_UP)`, then **clamped to `[0, 1]`** — `str()` first avoids binary-float artifacts (`Decimal(str(2/3))` = `"0.6666666666666666"`, not the 55-digit `Decimal(2/3)`), then quantize to the field's **4 dp** so the value *written* equals the value *read back*. The `fraction` field (`max_digits=5, decimal_places=4`) has **no headroom above `1.0000`**; the marking contract is `0 ≤ fraction ≤ 1`, which is unenforced at the DB level, so the clamp at this boundary guarantees a future buggy `mark()` returning `>1`/`<0` can't raise on save (it's bounded, not silently trusted). This stored 4dp `f` is the **only** fraction used downstream — both the per-attempt `earned_marks` and the §3.4 Finish recompute read it, so they are identical by construction (resolving any "stored vs raw" desync).
...
**Resume obeys the withhold rule (no-leak).** An answered-but-not-yet-correct `[A]` question (wrong, attempts remaining) rehydrates into the **withhold-gated** feedback state of §3.2 — the prior "Incorrect — try again (N left)" message **without** `reveal`/`reveal_template` — exactly as the live `quiz_answer` response would. Rehydrating `latest_answer` must **not** route through a feedback path that reveals: the resume render builds the same reveal-gated context (§3.2), so the accepted answer never reaches the page for a still-in-progress question. Only correct/exhausted (locked) `[A]` questions render the reveal on resume.
