...
- **One canonical pool builder.** A single pure function **`dnd.build_pool(question)`** — a referentially-transparent function of stored data only (gap/right tokens + distractors), de-duplicated by `normalize_text`, with **no per-request randomness or session seed** — is the sole source of the pool. It returns the pool as an **ordered list** in a deterministic, non-revealing order (sorted by `normalize_text`). **Both `render` and `mark` call the same `build_pool`** (render to emit `<option>`s in that order; mark to validate membership), so the two can never disagree, and because correctness is by text the order is purely presentational and never affects scoring (it is fixed here only so the option order isn't a giveaway and doesn't reflow between submissions/resume).
...
- **Distractors:** the pool may contain extra tokens that are the correct answer for no target (they are pool members that no target expects). Distractors enter the pool via `build_pool` exactly like correct tokens.
...
- **drag-fill** (`dragfillblankquestion.html`): the token-stem is rendered with a `render_selects(token_stem, pool, chosen)` helper (a sibling of `fillblank.render_inputs`) that splits on the opaque tokens and emits, per gap, a `<select name="slot">` whose `<option value>` is each pool token's **text** (`build_pool` order, §1.2), with the option matching `chosen[i]` pre-selected on resume. Text segments are trusted sanitized HTML; only the server-built `<select>`s are inserted, with every `<option>` value **and** label HTML-escaped (`format_html`) — same safe-join discipline as `render_inputs`.
- **match-pairs** (`matchpairquestion.html`): a two-column layout — each `left` label beside its `<select name="slot">`.
...
- **`expected`**: the ordered list of raw expected-token strings, length `n_targets` (drag-fill: each `DragBlank.correct_token`; match-pairs: each pair's `right`).
- **`pool`**: the ordered raw-token list from `dnd.build_pool(question)` (§1.2) — used **only as a membership set** to reject forged input, not for index math.
- **`chosen`**: the per-target submitted token-text list (length `n_targets`, `""` for unfilled).
- Returns `(n_correct, reveal)`.

The flow:

- `build_answer(self, post)` → `post.getlist("slot")` — a list of submitted **token-text strings**, one per target, padded with `""` / truncated to exactly `n_targets` (mirrors fill-blank's pad/truncate). **Unfilled targets serialize as `""`** (never `None`/`0`), so the existing `quiz.answer_is_empty` list-branch (`not any(str(v).strip() …)`) treats an all-`""` answer as empty and **any** non-blank token text as non-empty.
- `mark(self, answer)` reconstructs `expected` from `self` and `pool` via `dnd.build_pool(self)` — the **same** function `render` uses (so membership can never disagree with what was rendered), then calls `mark_slots`:
  - For each target `i`: `got = answer[i]`. A `got` that is `""` **or not a member of `pool`** (forged/edited-away) counts as unfilled → wrong. Otherwise `ok = normalize_text(got) == normalize_text(expected[i])` — **both sides normalized**, so a pool token that was de-duplicated by `normalize_text` still matches a raw-distinct-but-normalize-equal expected token (§5.2 test).
...
- **Prefetch (perf).** `lesson_unit` and `quiz_unit` build `choice_qs`/`fill_qs` and call `prefetch_related_objects(fill_qs, "blanks")` (`views.py`). Add `dragfill_qs`/`matchpair_qs` prefetching `"dragblanks"`/`"pairs"` (or a base `prefetch_fields()` hook returning the relation names). Without this, `mark`/`render` rebuilding the pool from `self` triggers N+1 queries.
- **KaTeX detection.** `_question_has_math(q)` switches on `ChoiceQuestionElement`/`FillBlankQuestionElement` to decide whether to load KaTeX (`views.py`). Tokens carry KaTeX (§2.3), so both new types must report math — via a base `has_math()` hook (preferred) or added branches. Missing this means token math doesn't render.
...
- **Resume / JSON round-trip.** `quiz.rehydrate` and `quiz.answer_from_json` branch on `ChoiceQuestionElement` and **fall through to a default** that returns `latest_answer` unchanged (`quiz.py`). The new types correctly use that default (token-text list passes through untouched, §2.4) — but the spec **requires a test** pinning that they stay on the default path (a future refactor mustn't accidentally route them through the choice branch).
