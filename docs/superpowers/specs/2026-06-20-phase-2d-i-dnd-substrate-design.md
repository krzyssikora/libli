...
- **KaTeX detection.** `_question_has_math(q)` is **not** a method — it is a **local closure defined inside `build_lesson_context`** with hard `isinstance` branches (`views.py`), and the quiz path (`build_quiz_context`) needs the same logic. Introducing a `has_math()` hook therefore means **hoisting the closure into a method on `QuestionElement`/subclasses and replacing it in both `build_lesson_context` and `build_quiz_context`** (not merely implementing a method an existing dispatcher calls). KaTeX can appear in **tokens *and* labels** (§2.3), so each new type's `has_math()` scans **all** math-bearing fields: drag-fill → the `stem` + every `DragBlank.correct_token` + distractors; match-pairs → the `stem` + every pair's `left` **and** `right` + distractors. (Scanning only the pool would miss `left`-column math, which fill-blank has no analogue for.) Missing this means that math doesn't render.
