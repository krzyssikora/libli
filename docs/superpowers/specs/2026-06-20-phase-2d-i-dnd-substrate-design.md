...
- **Distractors:** the pool may contain extra tokens that are the correct answer for no target (they are pool members that no target expects). The `distractors` field is split by **`_lines(...)` — the existing `courses.models._accepted_lines` helper** (newline-delimited, blank lines dropped, each line **un-trimmed**), the same splitter fill-blank already uses. **A note on whitespace, since the two token sources differ:** gap/`right` tokens that come from a `{{…}}` marker are **already whitespace-trimmed** by the reused `fillblank.parse()` (`[p.strip() for p in …]`), so `DragBlank.correct_token` is the *trimmed* marker text; distractors from `_accepted_lines` are *un-trimmed*. `build_pool` stores each token **as it receives it** (no additional rewriting) and dedups **purely by `normalize_text` key** (which internally trims + collapses whitespace + casefolds, §`marking.py`). Because membership, matching, **and** dedup all key off `normalize_text`, this trim asymmetry is **invisible to scoring** (whitespace-insensitive throughout); it affects only which *raw* string a normalize-collision keeps as the displayed chip (resolved deterministically by the §1.2 tie-break: first occurrence, correct tokens before distractors). Distractors otherwise enter the pool exactly like correct tokens.
...
- **Pool** is built by `dnd.build_pool(self)` (§1.2) from `[b.correct_token for b in self.dragblanks.all()]` + `_lines(self.distractors)`. Each gap's expected token = its `DragBlank.correct_token` (the stored marker text, already whitespace-trimmed by `parse()`, §1.2; marking normalizes both sides, §3.2).
- `correct_token` is `CharField(max_length=500)`, but the value is extracted from the unbounded `stem` textarea, so the form **validates token length ≤ 500 before create** (§4.1) to turn an over-long token into a friendly form error rather than a DB `DataError`.
...
- `build_answer(self, post)` → returns the **raw `post.getlist("slot")`** — a list of submitted **token-text strings** — with **no pad/truncate** (matching fill-blank's actual layering: its `build_answer` is a bare `return post.getlist("blank")`, and length normalization to `n_targets` lives in `mark`/`mark_slots`, §below, via the defensive `chosen[i]`-for-`range(n_targets)` reading — *not* in `build_answer`). Each `<select>` submits `""` for an unfilled target (the empty placeholder, §1.2), so the list already carries `""` in unfilled positions; the existing `quiz.answer_is_empty` list-branch (`not any(str(v).strip() …)`) treats an all-`""` answer as empty and **any** non-blank token text as non-empty. `build_answer` returns a **`list`** (not a tuple) so `quiz.answer_to_json` passes it through unchanged (§2.4). (Note a no-JS submit always yields one `slot` value per rendered `<select>`, so the list length equals `n_targets` in normal operation; the `mark_slots` length-defense covers only forged POSTs and edited-after-store replays.)
...
**Case-sensitivity (deliberate):** `normalize_text` accepts a `case_sensitive` kwarg (fill-blank threads each `Blank.case_sensitive` into it), but drag-fill/match-pairs define **no** `case_sensitive` field and call `normalize_text` at its **default (case-insensitive)** — so all token matching here casefolds. This is intentional: a student picks a discrete chip from a pool, so case-variant distractors (`\(X\)` vs `\(x\)`) would be a confusing way to author a distinction; case-sensitive token matching is an explicit non-goal for 2d-i. (If ever needed, a per-target `case_sensitive` field can be added later, mirroring `Blank`.)
...
Two new `ModelForm`s in `courses/element_forms.py`, registered in `FORM_FOR_TYPE` as `"dragfillblankquestion"` and `"matchpairquestion"`. Wiring a question type through the authoring flow is **not** just the form + registry — it spans the add-menu template, two `views_manage` allowlists, the host-form/formset render path, and the `builder.save_element` persist branch; **all** authoring touchpoints are enumerated in §4.4 (the marking/render touchpoints are there too). Both forms use the existing `_MarkingFieldsMixin` (so `marking_mode`/`max_attempts`/`max_marks` appear, quiz-gated, exactly as 2c wired for fill-blank).

### 4.1 drag-fill-blanks form

- A `stem` textarea using the **`{{token}}` syntax**, parsed by the **reused `fillblank` pipeline**: `sanitize_html(raw)` → `strip_sentinel` → `parse()`. Following the fill-blank precedent, the form's **`clean_stem` does not create rows** — it parses and **stashes the per-marker tokens on the form** (e.g. `self.parsed_dragblanks`, the single-token analogue of `FillBlankQuestionElementForm.parsed_blanks`); the actual `DragBlank` rows are created by `builder.save_element` with a **delete-then-rebuild** step on edit (§4.4 / I-mirror of the fill-blank branch).
- **Its own `clean_stem`, not fill-blank's verbatim** — the messages differ: (a) a `FillBlankError` (no markers / unterminated) must surface a drag-fill message (*"Mark at least one gap with {{token}}."*) — **not** fill-blank's literal *"use | for alternatives"* hint, which is the opposite of the drag-fill rule; (b) after a successful `parse()`, the wrapper **post-checks each marker has exactly one piece** (`len(pieces) == 1`) and rejects `|` with *"Each gap holds one token — use a single answer per {{…}}, not alternatives."*; `correct_token = pieces[0]`.
- **Token length:** because a marker interior is extracted from the unbounded `stem` `TextField` (not a bound `CharField`), the wrapper also **validates each token ≤ 500 chars** with an i18n message, so an over-long token surfaces a form error rather than a DB `DataError` at `DragBlank.create` (the `max_length=500` cap, §2.1). (Match-pairs' `left`/`right` are bound `CharField(500)` on the formset `ModelForm`, so Django enforces their length automatically — no extra check needed there.)
- A `distractors` textarea (newline-delimited extra tokens), `blank=True`.

### 4.2 match-pairs form

- An **inline formset** of `MatchPair` rows (`left`, `right`, `order`), mirroring how `Choice` rows are authored for MCQ (add/reorder/delete, ≥1 pair required) — including the **same host-form/formset plumbing** the choice type uses: a `build_matchpair_formset` (sibling of `build_choice_formset`), threaded through `_render_open_form` and re-bound on the 422 invalid-re-render path, and rendered by the host-form template. The choice type is special-cased by `type_key == "choicequestion"` at every one of these points; match-pairs must be added alongside it (§4.4), not assumed to flow through generically.
...
Marking and rendering dispatch polymorphically, but a question type is wired into the platform through **many hard `isinstance` branches, enumerated lists, and `type_key` allowlists** across `courses/views.py`, `courses/quiz.py`, `courses/views_manage.py`, `courses/builder.py`, and two editor templates — none of which pick up a new `QuestionElement` subclass automatically. Each new type must be wired into **all** of them (confirmed against the post-2c tree); a missed one fails silently or with a "bad type" 400. "No new view functions" (§1.4) is literally true but the existing-code edit surface is **substantial** — the **consumption** touchpoints are listed first, then the **authoring** touchpoints. Where a branch recurs (KaTeX, prefetch), the **preferred** approach is to push it behind a small hook on the `QuestionElement` base; enumerated edits are the fallback where a hook is overkill.

**Consumption touchpoints (`views.py` / `quiz.py`):**
...
- **KaTeX detection (lesson path only).** `_question_has_math(q)` is **not** a method — it is a **local closure inside `build_lesson_context`** with hard `isinstance` branches (`views.py`). The **quiz path does *not* need touching for math:** `build_quiz_context` already sets `has_math = bool(questions)` (deliberately over-inclusive — KaTeX loads whenever the quiz has any question, per its own comment), so quiz token/label math always renders. Only the **lesson** path does precise per-stem detection, so only `build_lesson_context`'s closure must gain branches for the two new types (or, preferred, be hoisted to a `has_math()` method on `QuestionElement`/subclasses). KaTeX can appear in **tokens *and* labels** (§2.3), so each new type's detection scans **all** math-bearing fields: drag-fill → the `stem` + every `DragBlank.correct_token` + distractors; match-pairs → the `stem` + every pair's `left` **and** `right` + distractors. (Scanning only the pool would miss `left`-column math, which fill-blank has no analogue for.) The failure mode if missed is narrow: a **lesson** page whose **only** math lives in a drag-fill/match-pairs token or label would not load KaTeX (`has_math` is an OR across all elements, so a co-present `MathElement` or other math question would mask the omission in casual testing). (If precise quiz-side detection is ever wanted, swapping `bool(questions)` for the hook is a separate, behavior-changing, N+1-introducing decision — out of scope for 2d-i.)
- **Question-CT gate (lesson path only).** `question_models` / `question_ct_ids` is a hard-coded model list backing `has_questions` in **`build_lesson_context` only** (`views.py`); `build_quiz_context` hardcodes `has_questions = True`, so the gate has no effect there. Add both new models to the lesson-path list (or derive it from `QuestionElement.__subclasses__()`); the quiz path needs no change.
...
**Authoring touchpoints (`views_manage.py` / `builder.py` / templates):**

- **Add-element menu.** `templates/courses/manage/editor/_add_menu.html` is a static list of `data-add-type` buttons (e.g. `fillblankquestion`). Add **two** buttons — `data-add-type="dragfillblankquestion"` and `data-add-type="matchpairquestion"` — each with an icon + i18n label. (Note: choice maps `choice-single`/`choice-multi` → `choicequestion` in `views_manage`; the new types' `data-add-type` is their `type_key` directly.)
- **`type_key` allowlists.** `views_manage.element_add` and `views_manage.element_save` each have a hard-coded `if type_key not in (… "fillblankquestion"): return HttpResponseBadRequest("bad type")`. Both new keys must be added to **both** allowlists, or the open/save is rejected with a 400 before the form runs.
- **Host-form / formset render (match-pairs only).** `_render_open_form` and the element-edit open path special-case `type_key == "choicequestion"` to build the inline formset (`build_choice_formset`) and thread it into the template; the invalid-re-render (422) path re-binds `e.formset`. Match-pairs needs the analogous `build_matchpair_formset` wired at **all three** points (open, edit-open, 422-re-render) and rendered by the host-form template. Drag-fill has no formset (it uses the `stem` textarea), so it needs none of this.
- **`builder.save_element` persist branch.** Sub-row creation is a hard `elif type_key` chain (`builder.py`): `choicequestion` drives the formset save; `fillblankquestion` does `form.save()` → `obj.blanks.all().delete()` → recreate `Blank` rows from `form.parsed_blanks`; the `else` just `form.save()` with **no** sub-rows. Add **two** branches: drag-fill mirrors the fill-blank one (`form.save()` → `obj.dragblanks.all().delete()` → recreate `DragBlank` from `form.parsed_dragblanks`, the delete-then-rebuild keeping edits clean); match-pairs mirrors the choice formset-save branch. Without these, a new type saves the element but **zero sub-rows**.
