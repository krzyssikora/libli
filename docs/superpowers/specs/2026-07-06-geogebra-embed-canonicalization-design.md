...
everything else through untouched. It is wired into `extract_embed_url` at the
single point where the final URL string is known.
...
**Host recognition:** the host equals `geogebra.org` or `www.geogebra.org`, or
ends with `.geogebra.org` (subdomains included — mirrors the allow-list matcher).
...
Implementation note: the two families reduce to two checks — (a) segments start
with `m` → take the next segment; (b) `id` appears in segments → take the
segment immediately following the first `id`. Query string and fragment are
ignored.

**ID validation:** the extracted candidate must match `^[A-Za-z0-9]+$`.
...
- `*.geogebra.org` subdomain recognized;
...
**Three existing `tests/test_embed.py` expectations change intentionally (not
regressions):**

- `test_plain_https_whitelisted_url_passes_through`: `https://www.geogebra.org/m/abc`
  now canonicalizes to `https://www.geogebra.org/material/iframe/id/abc`.
- `test_valid_snippet_extracts_src`: the `.../id/abc123/width/800/height/600`
  fixture now yields `.../id/abc123`.
- `test_iframe_form_stores_only_src`: same fixture → stored URL is `.../id/abc123`.
