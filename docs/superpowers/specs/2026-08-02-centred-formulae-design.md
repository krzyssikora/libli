...
- Full non-e2e suite: **4568 total**, measured on `master` at `671c57f0` in the implementation
  worktree — `4568/5226 tests collected (658 deselected)`, 4568 passed / 0 skipped in 221 s at
  `-n 4`. (An earlier draft of this section said 4566, carried over from a measurement on a
  different commit; corrected here after plan-review re-measured it.) `test_db_quiesce.py` carries a
  dynamic skip that runs as a pass depending on session state, so treat the **total** as the
  invariant, not the pass/skip split. Every test this slice adds is e2e, so the non-e2e total should
  not move.
