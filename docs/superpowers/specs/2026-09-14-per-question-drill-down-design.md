...
- It does **not** filter status, so it works unchanged on an IN_PROGRESS submission.
...
- `answer_from_json` (`courses/quiz.py:192-198`): choice → set of pks; everything else unchanged.
  ...
  (`:2842-2856`).
- `CONCRETE_QUESTION_MODELS` (`courses/richtext.py:26`) is a hand list pinned by
  `len(...) == 10` in `tests/test_richtext.py:273` and `tests/test_richtext_drift.py:167`.
  §4.4's guard must not become a third count pin.
...
- **Status:** the pill (D8), computed by the breakdown's own rule for this one submission:
  `_quiz_review_maps([unit.pk], [submission])` + `submission_is_counted` +
  `_pct` (`courses/rollups.py:337-409`, `:514-532`). Kinds and what each shows:
  - `scored` — "scored s/m (p%)", as in the breakdown;
  - `submitted` — submitted but ungraded (`max_score` 0);
...
class Part:
    ...
    given: str | None      # the pupil's value as display text; None = not answered
...
3. **Unanswered** (`response is None` or `latest_answer is None`): every part has `given=None`;
   for AUTO, `ok=False` and `expected` filled, from the empty-answer `mark_result`.
4. Per-part `given=None` also means "this part left empty" inside an answered question (an empty
   gap, an unselected grid row, an empty drop slot).
...
| extendedresponse | 1 for the text, then one per keyword | the text, line breaks preserved; keyword parts: `None` | keyword parts: `None`. Label `"Required: kw"` / `"Avoid: kw"` (existing msgids) | text part: `.correct`; keyword part: `found` for Required, `not found` for Avoid — **`None` on every keyword part when unanswered**, as `_reveal_extendedresponse.html:18-28` does |
...
- `<ol class="answers__list">`, one `<li class="answers__item is-{{ row.outcome }}" data-question>`
  ...
  - the outcome badge, **reusing `quiz_results.html:31-37`'s markup and msgids** (Correct,
    Partial, Incorrect, Not answered, Answer recorded, Reviewed, Awaiting review);
  - for REVIEW rows on an `awaiting` submission, a "Review" link to the review page;
  - the parts: one `<div class="answers__part">` each — the label if any; `given`, or muted
    "Not answered" (existing msgid) when `None`; a ✓/✗ glyph when `ok` is not `None`, `aria-hidden`,
...
`courses/_katex_js.html`, as `analytics_student.html:4,19` does, and mark each item
`data-question` so the typesetting pass reaches it, as `quiz_results.html:28,62-69` does.
...
- `docs/help/teacher/drill-down.md` + `.pl.md`: a "Per-question answers" section after
  ...
  committed PNGs on this machine's renderer.
...
assertions to the item or header element, not the whole page: pupil names and quiz titles repeat
in the breadcrumb and back link.
...
- **T30 Access (parent §8 — PR 5 does not merge without it).** One fixture: a course with two
  ...
  Mutants: gate on `request.user.is_staff` instead of `can_review_course` (the staff row goes
  red); replace step 3's `reviewable_students(...)` with an unscoped `User` lookup (the
  other-group row goes red).
- **T31 Resolution.** Against the owner, each case changes exactly one path segment of a URL
  asserted 200 in the same test: a node from another course → 404; a lesson node of the same
  course → 404; a non-existent pupil pk → 404; a pupil enrolled with no submission on this quiz →
  404; a **draft** quiz with a submission, viewed by the pupil's group teacher → 200.
  Mutants: drop `require_quiz=True`; add `viewer=request.user` (the draft case goes red — a group
  teacher cannot see drafts); treat a missing submission as an empty page.
- **T32 Answer summary, per type.** For each of the ten types, pure calls to `summarise` asserting
  ...
  extendedresponse covers unanswered keyword parts (`ok is None`).
...
- **T33 Content drift.** fillblank with fewer and with more stored values than current blanks;
  ...
  pk. Mutants: skip the padding (IndexError or misaligned parts); read choicegrid `given` from
  `reveal["chosen_label"]` (the removed pk shows as not answered).
- **T34 Drift guard** (§4.4). Falsified twice: delete one registry entry; and, separately, define a
  throwaway concrete `QuestionElement` subclass in a test-only app (or patch `apps.get_models` to
  include one) so the derived set grows.
...
- **T36 Header parity with the breakdown.** For each of the four submission states, the new
  ...
  SUBMITTED quiz with an unreviewed REVIEW question (`awaiting`, with the Review link) and a
  SUBMITTED ungraded one (`submitted`). Mutant: show the score for an awaiting submission.
- **T37 Breakdown links.** The quiz title is a link to the new page for scored, submitted,
  ...
  `values`. Mutants: link not_started; drop `values` from the back link.
...
- **T39 Maths.** A pupil's short-text `given` containing `\(x\)` in a quiz with no other maths
  sets `has_math`. Mutant: compute `has_math` from stems only.
...
- **Existing tests that move:** `tests/test_analytics_views.py`'s breakdown tests
  (`:199-291`) keep passing unchanged — the pill partial extraction must not change their
  asserted text (`90%`, the `/review/<pk>/` link). `courses/tests/` covering `build_quiz_context`
  keep passing after the prefetch extraction.
