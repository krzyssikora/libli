...
- `lesson_unit.html` overrides `{% block prepaint %}` (inline `reveal-armed` setter + watchdog — no `{{ block.super }}` needed, the base block is empty) and `{% block extra_css %}` (render-blocking pre-hide `<style>`, after app.css), and adds the deferred `reveal.js` in `extra_js` — **all three gated on `has_reveal_gate`** (the co-gating triad). See Data flow for exact placement/cascade rationale.
...
- **Preview-pane behavior (accepted):** in the editor's Preview mode the gate button is server-`hidden` and no engine runs, so the gate has no visual in the Preview pane (content shows fully expanded, matching the no-JS student view). This is acceptable by design — the **Editor (element-list) view's bespoke row is the authoring surface** for the gate; the Preview pane intentionally does not surface it. The row detects quiz-ness by comparing `unit.unit_type` to `ContentNode.UnitType.QUIZ` (`_element_row.html` already receives `unit`); if the enum is not reachable from the template, pass a boolean into the row's context.
...
- **Co-gating invariant (fail-safe).** The three pieces — the inline `reveal-armed` setter, the render-blocking pre-hide `<style>`, and `reveal.js` — form an **all-or-nothing triad emitted together under one condition**: a `has_reveal_gate` context flag on the lesson page (mirroring the existing `has_math` / `has_questions` / `has_html` gating in `lesson_unit.html`). **`has_reveal_gate` must be true whenever a gate exists *anywhere* in the unit, including nested inside a tab** — compute it as a **flat** query over all `Element` rows for the unit whose concrete type is `RevealGateElement` (child elements keep their own `unit` FK, so a flat query catches tab-nested gates without a recursive walk). A lesson whose *only* gate is inside a tab must still emit the triad, or that gate's button is never un-hidden. NEVER emit the setter or the pre-hide `<style>` without `reveal.js`: the pre-hide would then hide content under `<html class="reveal-armed">` with no engine to reveal it and no visible button (buttons stay server-`hidden`) — a fail-closed content swallow. **Placement (cascade- and paint-safe).** The inline `reveal-armed` **setter + watchdog** go in base.html's `{% block prepaint %}` (`base.html:43`, in `<head>` *before* the stylesheet links — must run synchronously, pre-paint; NOT the deferred `extra_js`, which would reintroduce the flash). The render-blocking pre-hide **`<style>`** goes in `{% block extra_css %}` (`base.html:48`, **after** the `app.css` `<link>` at `base.html:46`) so its plain-`display:none` rules win any equal-specificity clash with app.css while staying in `<head>` (still pre-paint). Both blocks already exist, so **base.html needs no change**. `{% block prepaint %}` is **empty** in base.html (the app-wide unit-tree pre-paint script sits *outside* it, `base.html:34–42`), so the `lesson_unit.html` prepaint override needs **no `{{ block.super }}`** — omitting it drops nothing. The pre-hide selectors are already high-specificity (class + `:has()` + combinators); still, verify no app.css rule sets `display` on `.slide > .lesson-block` / `.tabs__child` at comparable specificity (a clash would fail *open* — un-gated — not swallow content).
