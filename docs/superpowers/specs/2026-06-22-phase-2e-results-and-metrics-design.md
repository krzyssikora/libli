...
- **Per quiz unit**, build a **row** dict: `{ "unit": node, "status": <str>, "graded": <bool>, "score": <Decimal|None>, "max_score": <Decimal|None>, "pending": <bool>, "url": <str>, "url_name": <str> }`. **`graded`** = `True` iff the row has a numeric, non-zero possible total (`max_score is not None and max_score > 0`); it discriminates a normally-marked `submitted` row (`graded=True`, render `X/Y`) from a fully-`[N]` `submitted` row (`graded=False`, render the "submitted — not graded" cue) — see §3.3 / I2-I3. `not_started`/`in_progress` rows are `graded=False` (no score yet).
