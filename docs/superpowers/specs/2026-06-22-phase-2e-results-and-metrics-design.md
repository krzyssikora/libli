...
- **Quiz-unit enumeration & ordering (pin the flatten).** Quiz units are emitted in the **depth-first pre-order** of the content tree — the order they appear walking the outline top-to-bottom. **Critical implementation note:** `course.nodes.all()` is a *flat, globally-sorted* queryset (sorted by `ContentNode.Meta.ordering = ["order", "pk"]` — models.py:109–110) across **all** depths at once; a **linear iteration of that flat queryset is NOT depth-first pre-order** and is **not** a valid implementation (sibling `order` values are only locally monotonic — `OrderField(for_fields=["course", "parent"])`, models.py:99 — so a chapter `order=0` with a child section `order=5` and a sibling chapter `order=1` interleave wrongly in a flat scan). The flatten **MUST recurse the `parent_id`-grouped structure**, i.e. the exact algorithm `build_outline` uses (group nodes by `parent_id`, recurse children in `Meta.ordering`). Concretely: either (a) call a small shared pre-order flattener over `build_outline`'s nested result (filtering to quiz units), or (b) replicate `build_outline`'s `parent_id`-grouped recursion — **never** a flat `course.nodes.all()` scan. **The pinned, tested property is that row order equals the depth-first pre-order quiz-unit sequence of the outline**; two implementers must produce the same order.
- **Query budget — the invariant is N-independence, not a hand-counted integer.** The helper's query count must be **independent of the number of quiz units, submissions, and responses** (no per-row/per-unit N+1); it grows **only** with the number of distinct question **content-types** present (the GFK batch). The base queries are: **(1)** the course's nodes; **(2)** the student's `QuizSubmission`s (`filter(student=student, unit__course=course)`), indexed `{unit_id: submission}`; **(3)** the student's `QuestionResponse`s for those submissions in one query (`filter(submission__in=…)`), **with the `Element` join folded in via `select_related("element")`** so it does **not** cost a separate query; then the GFK content_objects via `prefetch_related("element__content_object")` (one query per content-type — see next bullet). **Do not assert a hand-counted integer in the spec** — `prefetch_related` spanning an FK-then-GFK can add a query depending on whether `select_related("element")` lets Django satisfy the relation from the responses query; the **test author derives the exact `N` empirically** (read what `django_assert_num_queries` reports for the fixture) and then pins it as a regression guard. What the spec pins is the *shape*: `N` stays constant when you 10× the rows and changes only when you add a new question content-type. (`build_outline`'s "two queries" is a *sibling precedent*, not a budget this helper must match — it does more.)
...
- **List:** one row per `summary.rows` entry, mirroring the outline's visual rhythm (bespoke token CSS, no new deps). Each row: quiz title, a status presentation, the score (`X/Y` for graded rows; "—" otherwise), and a link via `{% url row.url_name course.slug row.unit.pk %}`. **All four row targets take `(slug, node_pk=row.unit.pk)`** — `quiz_results` and `quiz_unit` are both `<slug>/u/<int:node_pk>/…` routes, and `row.unit` is the quiz `ContentNode` whose `.pk` is the `node_pk` (there is no "no-node_pk" target among the rows). **`course` here is the top-level context var the view passes** (§3.2), not `summary.course` — the helper also returns `course` inside its dict (handy for callers/tests), but the template references the explicit `course` context var to keep one canonical source. Status presentation:
  ...
  - `awaiting_review` → `{{ row.score|marks }} / {{ row.max_score|marks }}` + an "awaiting review" cue + "details" link. **The "⏳" glyph stays a template literal outside the `{% trans %}` wrapper** — only the text "awaiting review" is translated (don't bundle the emoji into the i18n string).
...
- **Unit/integration (pytest + factory_boy, real PostgreSQL):**
  ...
  - **Query budget — N-independence (not a hand-counted constant):** with a fixed fixture, record the **empirical** query count `N` (`django_assert_num_queries`) and pin it as a regression guard; then assert that **10×-ing the number of quiz units/submissions/responses leaves the count unchanged** (no per-row/per-unit N+1), and that it rises only when a new question content-type is introduced. Do not assert a hand-derived integer — read what Django actually issues for the fixture (§3.1).
