...
- **The `node_add` view** applies the course-policy check **only to a resolved, structurally-real kind** — i.e. `kind in ContentNode.RANK and kind not in course.allowed_kinds` → reject by **reusing `node_add`'s existing `ValidationError` → status 422 render path** (`_builder_with_notice` / `_op_error.html`, `views_manage.py:237-243`) with a clear message (e.g. EN *"You can't add the {kind} level to this course."*, `{kind}` = the translated kind label — phrased as "the {kind} level" so the nominative `kind_label` fits cleanly in EN **and** PL, matching §2.2's approach to inflection; included in the EN/PL i18n test set). This runs **inside the existing `try:` block** (`views_manage.py:217`), immediately after `try:` and before the `builder.add_node(...)` call (`:218`) — so it acts on the *resolved* kind **and** its raised `ValidationError` is caught by the existing handler (`:237-243`) that renders the 422. (Placing it *before* the `try:` would escape the handler and yield a 500, not 422.) An empty (`""`) or unknown/garbage `kind` is **not** treated as a course-exclusion — it falls through unchanged to `builder.add_node` → `full_clean()`, which also raises `ValidationError` → the **same 422** path. So both branches return 422 and differ only in message, preserving today's malformed-POST semantics and tests. The check lives in the **view**, not in `builder.add_node` (which gains no `allowed_kinds` parameter) — so a forged or stale POST cannot bypass the policy, and the test hooks at the view layer.
...
- **`manage_node_add` server guard** — a forged POST adding an excluded kind (e.g. `part` in a Chapters course) returns **status 422** with the excluded-kind message (mirror the existing malformed-POST 422 assertions — this guards against a misplaced check that would yield 500); an allowed kind still succeeds.
