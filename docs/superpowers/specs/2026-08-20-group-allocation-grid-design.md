...
| 33a | e2e: the click-from-indeterminate direction. **Setup is load-bearing** — `checked` must be genuinely `true` before the indeterminate transition, which a freshly loaded page never is: apply the filter, click add-all so all visible students are ticked (assert add-all is now `checked`), untick **one** visible student, then click add-all and assert the remaining visible students end up **ticked, not cleared**. Asserting `input.checked is False` while `indeterminate` is `True` right after the untick reads the invariant directly and is equally acceptable | leave `checked` untouched in the indeterminate branch, so the browser's flip clears the visible selection instead of extending it |
...
| 36a | e2e on a **freshly loaded** create group form (two manageable courses, each with an allocation, no course chosen yet): every `<optgroup>` is hidden and "— none —" is the only selectable option — asserted by the same DOM technique as 37a, never `bounding_box()`. Without this row, an implementation that binds only to `change` satisfies 37a (which drives a change first) while the freshly loaded form discloses every manageable course's allocations | bind `initAllocationFilter` to `change` only, omitting the initial pass |
...
| 37a | e2e on the **create** group form: changing the course select hides the non-matching `<optgroup>`s and resets a now-stale allocation selection to `""`. **Assert the hiding via the DOM, not `bounding_box()`:** the options and optgroups of a collapsed `size=1` select are not laid out in the page (their popup is browser UI), so `bounding_box()` returns `None` for hidden and visible options alike and that half of the assertion would be vacuous. Evaluate e.g. `[...select.querySelectorAll('optgroup')].filter(g => !g.hidden).map(g => g.label)` and assert it equals exactly course B's label. (The `bounding_box()` technique stays correct for rows 30c and 35, which target a `<label>` and a `<tr>`.) **Setup is load-bearing:** act as a PA (or a CA owning both courses) with **two** courses, each carrying a non-archived allocation — select course A, pick one of A's allocations, then switch to course B. On a single-course fixture there is no non-matching optgroup to hide and no way to make a selection stale, so *both* mutants survive | omit the reset (a hidden-but-selected option then posts a mismatched allocation); separately, append `initAllocationFilter` to the file without invoking it from the IIFE, so it never runs |
