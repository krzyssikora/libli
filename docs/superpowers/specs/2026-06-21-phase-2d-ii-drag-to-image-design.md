...
- Renders, **below** the image, the numbered list of `<select name="slot">` via a **new template tag** in `courses/templatetags/courses_extras.py` — a sibling of the existing `fill`/`match_rows` tags (`courses_extras.py:79,88`), which themselves call `dnd.build_pool(el)` and pass it to the helper. The new `zone_rows` tag does the same: `return dnd.render_zone_selects(list(el.zones.all()), dnd.build_pool(el), submitted_values)`. **The pool is built inside the template tag (mirroring the other DnD tags), NOT inside `render()` and NOT in the template itself.** The helper `dnd.render_zone_selects(zones, pool, chosen)` emits an `<ol class="dnd__rows">` of `(badge number, <select>)` rows in `zones` order, each `<select>` built by the existing `dnd._render_select(pool, chosen[i])` (leading "— choose —" placeholder, normalize-aware pre-selection, every option value+label `format_html`-escaped). `name="slot"` is uniform with the other DnD types, so `build_answer` is `post.getlist("slot")`.

Two new presentation hooks: the substrate helper `dnd.render_zone_selects` and the `zone_rows` template tag that feeds it the pool. The helper is **modeled on `render_match_rows`** (not an exact copy): each row contains **only** the badge number + `<select>` — no `left` label and no geometry (geometry lives on the badges, §above). **Index is the join key:** the nth badge (`data-zone="n"`), the nth `<select>`, and `expected_tokens()[n]` all refer to the same zone (all built from `self.zones` in `["order","pk"]` order); the JS zips badge[i] ↔ select[i] by index.
