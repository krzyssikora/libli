...
- **KaTeX actually renders in the quiz path (confirm, not assume).** Because `build_quiz_context` loads KaTeX whenever the quiz has any question (`has_math = bool(questions)`), a drag-to-image question with KaTeX in `correct_label`/`distractors` gets the KaTeX runtime on the page. The remaining requirement is that the **chip pool, the `<select>` option labels, and the reveal partial** all live inside the KaTeX auto-render scope (the same wrapper the other DnD types' chips/options/reveal sit in) so `\(…\)` is typeset there. **An e2e assertion (§7.4) confirms KaTeX renders in a quiz drag-to-image's chips, select options, and reveal**, not only in lessons — this is a verification, not a new code path.
...
### 7.5 Migration
One migration — **`0018_dragtoimage` with `dependencies = [("courses", "0017_dragfill_matchpair")]`** (0017 is the latest on this branch) — adds `DragToImageQuestionElement` + `DragZone`. No alteration to existing tables (the marking fields are already on the base from 2c's 0015). Passes the existing migration-consistency gate (`makemigrations --check`).
