...
- Normalize the answer once via `normalize_text(answer)` (default, case-insensitive). For each keyword, `present = bool(re.search(r"(?<!\w)" + re.escape(normalize_text(kw)) + r"(?!\w)", norm_answer))`. **Both sides are whitespace-collapsed by `normalize_text` before the boundary search**, so a phrase keyword (`"French Revolution"` → `"french revolution"`, single space) still matches an answer containing `"French  Revolution"` (double space) or a newline between the words — inter-word spacing/newlines never defeat a phrase match.
...
`ExtendedResponseQuestionElement.mark(answer)` returns `MarkResult(correct=correct, fraction=fraction, reveal=reveal)` where `reveal` is the §3.1 tuple. **Construct by keyword, never positional-splat:** `mark_keywords` returns `(fraction, reveal, correct)` but `MarkResult`'s field order is `(correct, fraction, reveal)` — a `MarkResult(*mark_keywords(...))` would silently swap `correct`↔`fraction` (a `bool`/`float` swap no type checker catches). The call site unpacks then names: `frac, reveal, correct = mark_keywords(...); return MarkResult(correct=correct, fraction=frac, reveal=reveal)`. The `fraction` is a `float`; the quiz path's 2c `Decimal` boundary (`scoring.py`) quantizes `fraction × max_marks` unchanged. Forged/over-cap input cannot error — `build_answer` caps and `mark_keywords` only searches text.
...
- **`answered`-flag plumbing (pin all three paths — the partial cannot see the flag otherwise).** (1) **Results page:** `_results_row` sets `row["answered"] = response is not None and response.latest_answer is not None` (the exact predicate already at views.py:640), and `quiz_results.html`'s reveal `{% include row.reveal_template … %}` (line 27) gains `answered=row.answered`. (2) **Live per-question reveal:** that path always follows a real submit, so `ExtendedResponseQuestionElement` **overrides `feedback_context`** to call `super().feedback_context(mark_result)` (the base returns `el`/`mark_result`/`reveal_template` — models.py:399) and add `answered=True` (this also satisfies §3.4's quiz `[A]` reveal, which renders via `quiz_feedback_context` → `question.feedback_context`). (3) **Fail-safe:** `_reveal_extendedresponse.html` treats a **falsy-or-undefined `answered` as "not answered" (neutral guide)** — so a missing-context include degrades to the safe neutral form, never to a false green ✓. (No other reveal partial reads `answered`, so the base `feedback_context` of other types is unaffected.)
...
- **Quiz total footer.** `pending_count`/`pending_marks` are **initialized to 0 before** the existing `for el in node.elements…` loop in `quiz_results` (views.py:583) and incremented **inside** it — but only **after** the loop's existing `if not isinstance(q, QuestionElement): continue` guard (line 585), since `q.marking_mode` would `AttributeError` on a non-question `content_object`. For each surviving element with `q.marking_mode == REVIEW`, increment `pending_count` and add `q.max_marks` to `pending_marks`, **counting every `[R]` element regardless of whether a `QuestionResponse` exists** (an unanswered `[R]` still counts as pending, per §1.5). `max_marks` is a **non-nullable** `DecimalField(default=Decimal("1"), MinValueValidator(0.01))` (models.py:332), so even an `[R]` row authored via the hidden-field path (where `_MarkingFieldsMixin` omits `max_marks` from POST and the model default applies) carries a numeric value — `pending_marks` never adds `None`; a test pins that an `[R]` element's `max_marks` feeds the tally. The two new keys go into the **existing** `render(...)` context dict (not a second pass).
- **Footer placement (load-bearing for the all-`[R]` quiz).** The footer renders **outside / independent of** `quiz_results.html`'s `{% if submission.max_score %}` … `{% else %}"Score: —"` block (lines 8–12), placed immediately after it and gated **solely on `pending_count > 0`**. This is essential because an **all-`[R]`/`[N]` quiz** has `max_score == Decimal("0.00")` (falsy → the score line shows "Score: —"), yet the pending tally is then the *only* meaningful signal and must still appear. The text is **"N questions awaiting review (up to M more marks)"**. Live-derived in the view, no stored field. (`[N]` questions are **not** counted as pending — intentionally ungraded, not deferred.)
