...
**Placeholder scan:** none. Every code step carries complete code. Two steps carry a conditional fallback (the `MultiGridQuestionElementFactory` existence check in Task 3 Step 2, the `node --check` availability in Task 4 Step 3) with the exact command to resolve them.
