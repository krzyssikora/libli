...
| swap the reveal-before-`place()` ordering (`place(); overlayImg.hidden = false;`) | `tall_source_is_clamped_and_lands_centred_at_360` — `place()` itself only writes `left`/`top`, but the *reveal between the two call sites* changes the box's measured size: with the swap, `place()` measures the caption-only box (~40px tall), computing `top = (900-40)/2 = 430` for a box that then grows to 884px once the image reveals. `fits` is false (bottom 1314 > 900) and `dy ≈ 422`, far outside the `<= 2` tolerance. Red on both assertions. |

**Three lines are deliberately unmapped.** Record them as such rather than writing rows that appear to test them:
