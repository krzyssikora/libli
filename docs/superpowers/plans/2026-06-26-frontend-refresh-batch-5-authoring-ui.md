...
3. a question editor showing the **marking-fields** box (`_marking_fields.html` — Marking mode / Max attempts / Max marks). **NOTE:** `_marking_fields.html` is wrapped in `{% if is_quiz %}` and the editor sets `is_quiz = unit.unit_type == QUIZ` (`courses/views_manage.py`), but `_seed_course_and_unit` hardcodes `unit_type="lesson"` — so on the default seeded unit the box renders EMPTY. For this item, seed a **quiz** unit: `ContentNodeFactory(course=<the seeded course>, kind="unit", unit_type="quiz", parent=None, title="Q1")` (or flip the seeded unit's `unit_type` to `"quiz"`), open ITS editor, then add e.g. `extendedresponsequestion`; the `.el-editor__marking-fields` box (and its `[data-marks-fields]` descendant) now renders. (Items #1/#2/#4 render fine on the default lesson unit — their own embedded marking box being empty there is irrelevant to what they verify.)
