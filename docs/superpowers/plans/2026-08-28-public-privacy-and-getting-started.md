...
3. **What we hold, and why** — account and identity (username, optional email address, display and real names, a register/student number a Platform Admin may enter -- `User.external_id`, written ONLY by
   the PA user-edit form, NEVER by SSO, and sent by the results webhook -- and, for SSO accounts,
   the identifier and claims the provider returns, stored as an allauth `SocialAccount` row); the learning record; groups and classes; your own notes, tags and uploads; preferences; support reports. **State explicitly that every answer you submit to a question is kept with its timestamp, not only your latest one.**
