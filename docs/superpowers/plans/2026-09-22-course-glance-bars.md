...
(first `grep -rn "landing-visual" templates` to confirm nothing else uses it). In the file this rule spans four lines — match its content, not the exact layout above. KEEP the `.landing-visual { display: flex; … }` rule directly above it: the cards rely on its `justify-content: center` and `gap`; only the `.card` descendant rule goes. Then, directly before the `@media (forced-colors: active)` block added in Task 3, add:
...
Expected: PASS. Then `uv run pytest tests/test_i18n_po_health.py $(grep -rlE "app\.css|tokens\.css" --include='test_*.py' . | grep -v test_e2e_ | grep -v '/.venv/')` (catalog hygiene + every test that reads the stylesheets, after this task's `app.css` edit).
...
(Any CSS/token fix was already committed at the end of Step 2.)
