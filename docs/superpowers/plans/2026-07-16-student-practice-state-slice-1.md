...
**Post-review coverage corrections** (plan-review rounds 1-2): the `[S1]` lesson-GET-200 parametrization (top-level / tabs / two-column) is in **Task 4**, not Task 3 — round 2 moved it, because `build_lesson_context` reads the removed `checklist_state` until Task 4 Step 3, so every lesson GET raises `AttributeError` until then. Container `eid` provenance is pinned by identity in Task 3; chapter- and section-level reset now drive the real view in Task 8; the anonymous endpoint case is in Task 5; the migration's tab-nested and empty/absent cases are in Task 2; and the interstitial's CSS + light/dark screenshots are in Tasks 8-9.
